...
Table~\ref{table:detection_postprocessing_comparison} shows results for the temporal activity localization task, where the effect of a mean smoothing filter can be seen, improving the localization performance. Figure~\ref{fig:example_results} shows some examples of the temporal localization prediction for some instances of the dataset, showing continuous and smooth segments.
